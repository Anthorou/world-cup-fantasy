...
function isTodayMatch(match: Match, today: string): boolean {
	const matchDate = new Date(match.date).toLocaleDateString("en-CA");

	return matchDate === today || isLive(match);
}
...
function getMatchStatusText(match: Match): string {
	...
	if (match.statusShort === "SUSP") return "Suspended";
...
export default function Fixtures(): React.ReactNode {
	...
	const todayMatches = displayedMatches
		.filter(match => isTodayMatch(match, today))
		.sort(sortMatches);

	const tomorrowMatches = displayedMatches
		.filter(match =>
			!isLive(match) &&
	...
	return (
		<section className="space-y-4">
			...
			{renderMatches("Today", todayMatches, expandedMatchId, setExpandedMatchId)}
			{renderMatches("Tomorrow", tomorrowMatches, expandedMatchId, setExpandedMatchId)}
		</section>
	);
}
...
function renderMatches(
	...
	setExpandedMatchId: (matchId: number | null) => void
): React.ReactNode {
	if (!matches.length) {
		return null;
	}
	...
	return (
		<div className="space-y-3">
			...
			{matches.map(match => {
				...
				const canExpand = live || isFinished;
				...
				return (
					...
					>
						...
						>
							...
							<div className="hidden items-center gap-4 sm:grid sm:grid-cols-[1fr_auto_auto_auto_1fr]">
								...
								<p className="text-3xl font-bold">
									{live || isFinished ? match.home.score : "-"}
								</p>
								...
								<p className="text-3xl font-bold">
									{live || isFinished ? match.away.score : "-"}
								</p>
						...
						{isExpanded && (
							<MatchDetails
								fixtureId={match.id}
								isLive={isActivelyPlaying(match)}
							/>
						)}
					</div>
				);
